  ...
  useEffect(() => {
    ...
    const startStreaming = async () => {
      ...
      if (Hls.isSupported() && !isIOS) {
        ...
        // นับจำนวน network error ติดต่อกัน สำหรับ exponential backoff
            ...
            switch (data.type) {
              ...
              case Hls.ErrorTypes.MEDIA_ERROR: {
                ...
                if (mediaErrorCount <= 3) {
                  console.log(`[HLS] Media error #${mediaErrorCount}, recoverMediaError`);
                  hls.recoverMediaError();
                } else {
                  // พยายามมากแล้วไม่ได้ → reconnect ใหม่ทั้งหมด
                  console.log('[HLS] Media error เกิน 3 ครั้ง, reconnect ใหม่');
                  cleanupHls();
              ...
              default:
                // Fatal error อื่นๆ → reconnect ใหม่พร้อม backoff
                console.log('[HLS] Fatal error, reconnect ใหม่ใน 5s');
                cleanupHls();
                setConnectionState('offline');
                setTimeout(() => {
                  if (active) setRetryCount(prev => prev + 1);
